...
abs(BIC(m2, m3)) #model 2 offers a better fit
```
The anova test shows that the 2 models are equal and from the AIC and the BIC we get that model 2 offers a better fit. The transformations suggested by boxtidwell were not very decent. 
...
m3 <- m3_aux
```
We will keep the new model3 as it offers a greater r-squared than model 2 and will have a better fit with residuals.
...
#from the step function we are left with these model
...
We will now consider adding an interaction between a factor and a covariate. To choose the best pairing, we should join the couple of variables with the worst correlation in other to create a new sort of variable that will give greater information to our model. Conversely, if we join 2 variables with strong correlation, we would be just adding redundant information because 1 variable explaing the other. From our previous analyses, we found that tax and manufacturer were not very correlated so we will pair those 2 variables.
```{r}

kruskal.test(df_clean$tax~df_clean$manufacturer)
boxplot(df_clean$tax~df_clean$manufacturer)
...
abs(AIC(m6, m4)) #model 5_v2 offers a better fit
abs(BIC(m6, m4)) #model 5_v2 offers a better fit
#model 6 = 0.8446
#model 4 = 0.8356
#model 5_v2 also offers a greater R2, so we will stay with model 5
...
hat.plot(m6)
```
As a priori observation of influentials we will use the hat value that is the most common measure of leverage. From the hat plot generated we can see we have a lot of observations with high leverage.

```{r}
ols_plot_cooksd_chart(m6)
```
As a posteriori observation of influentials we will use the cook's distance that is the most common measure of leverage. From the hat plot generated we can see we have a lot of observations with high leverage.

Finally to choose the infulential observations we will use Cook's distance because it offers a consistent method for detecting them. We could use a mixture of hat values (leverage obs.) and rstudent residuals (outliying obs.) to predict influential observations, but it is not as efficient and reliable.
```{r}
...
length(ll_rst)
ll_hat_rst <- unique(c(ll_priori_influential,llres))
length(ll_hat_rst)
...
par(mfrow=c(1,1))
plot(allEffects(m5), selection=1)
plot(allEffects(m5), selection=2)
plot(allEffects(m5), selection=3)
plot(allEffects(m5), selection=4)
plot(allEffects(m5), selection=5)
plot(allEffects(m5), selection=6)
```
We see that our model defines the following:

* the log(price) decreases in a linear way as the mileage grows
* the log(price) increases in a logaritmic way as the tax grows
...
We can see from our plots that the model fits perfectly.
...
```{r}
res.pca <- PCA(df[c(5,7,8,13)], graph=FALSE )
plot(res.pca$ind$coord[,1], log(df$price), pch=19,col="darkblue", main="PCA of variables vs Actual/Predicted price")
points(res.pca$ind$coord[,1], m3$fitted.values, col="red", pch=20)
