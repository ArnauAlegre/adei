...
library(knitr)
library(formatR)
knitr::opts_chunk$set(echo = TRUE, tidy.opts=list(width.cutoff=70),tidy=TRUE, warning = FALSE, message = FALSE)
```
...
As with all variables describing money, we can improve the distribution of our variable by applying a logarithmic transformation.
```{r}
hist(log(df$price),100,freq=F,col="darkslateblue",border = "darkslateblue")
mm<-mean(log(df$price));ss<-sd(log(df$price))
curve(dnorm(x,mean=mm,sd=ss),col="red",lwd=2,lty=3, add=T)
shapiro.test(log(df$price))
```
Despite our transformation, we still don't achieve a normal distribution, as the sw test p-value is lower than 0.05 . 
...
m3<-lm(log(price)~poly(mileage,3)+tax+I(mpg^-1)+poly(years_after_sell,2),data=df)
summary(m3)
```
From the summary, we can see that all monomials from the polynomial transformations are relevant. 
...
```{r}
anova(m2, m3)
AIC(m2, m3) #model 3 offers a better fit
BIC(m2, m3) #model 3 offers a better fit
```
...
#we see from the boxtidwell output that the mileage p-value from mileage is greater than 0.05 so the variable would not need a transformation. We can also see that the tax should be square rooted
boxTidwell(log(price)~mileage+sqrt(tax)+I(mpg^-2)+I(years_after_sell^2),data=df_clean)

m3_aux<-lm(log(price)~mileage+sqrt(tax)+I(mpg^-2)+poly(years_after_sell,2),data=df_clean)
summary(m3_aux)
m3 <- m3_aux

```
We will keep the new model3 as it offers a greater r-squared than model 2 and has a better fit with residuals.
...
```{r}
residualPlots(m3,id=list(method=cooks.distance(m3),n=5))
```
We can see that all the variables from the model show linear residuals because we have applied the according transformation to all the variables. We can also identify some influential observations with cook's distance. We will treat those later.
...
par(mfrow=c(1,1))
```
We can see that with this new model can be considered homoskesdastic, and it is also normally distributed.
...
m5 <- lm(log(price) ~ mileage + sqrt(tax) + I(mpg^-2) + poly(years_after_sell,2) + fuelType + engineSize + transmission + manufacturer + engineSize:manufacturer + fuelType:engineSize+ fuelType:manufacturer + fuelType:engineSize:manufacturer, data=df_clean)
...
m5 <- lm(log(price) ~ mileage + sqrt(tax) + I(mpg^-2) + poly(years_after_sell,2) + fuelType + engineSize + transmission + manufacturer + engineSize:manufacturer + fuelType:manufacturer + fuelType:engineSize:manufacturer, data=df_clean)
```
...
AIC(m6, m4) #model 4 offers a better fit
BIC(m6, m4) #model 4 offers a better fit
#model 6 R2 = 0.843
#model 4 R2 = 0.8357

```
Model 4 also offers a greater overall fit as the AIC & BIC scores are greater, but model 6 offers a light improvement over the residual squared score. Overall model 4 is better, as it is way smaller and simpler than model 6, so we will stay with model 4.
...
m7_hat_rst<-update(m4, data=df_clean[-ll_hat_rst,])
summary(m7_hat_rst)

m7_cook <- update(m4, data=df_clean[-ll_posteriori_influential,])
summary(m7_cook)
...
plot(allEffects(m7), selection=8)
```
 
We see that our model defines the following:

* the log(price) decreases in a linear way as the mileage grows
* the log(price) increases as tax increases in a logarithmic way
...
par(mfrow=c(1,1))
```
In the residual plots we can see the same patter as before, the model presents homoskedasticity and normality and fits the linear hypothesis. This suggests a level of remarkable consistency with the predictions of the model. We can also see the effect of the deletion of influential data as there are not outlying residuals.
...
```
From a look at the influence plot, we can see a clearly see difference from the influence plot of model6. We can see now that we do not have any influential observation. We only have some obs. with some leverage but without being outliers and vice versa.
...
```
We get as a result a 95% confidence interval of [16601.3 to 29954.19] with the actual predicted price being 22299.74 real price being £18,950 so our model has an error of 11.5%, this is more or less the expected error because of our model expains 87.77% of the variability.
