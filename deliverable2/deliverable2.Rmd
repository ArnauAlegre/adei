...
df$f.price<-factor(cut(df$price/1000,breaks=c(quantile(df$price/1000,seq(0,1,0.1),na.rm=TRUE)),include.lowest = T ))
...
while(res.km$betweenss/res.km$totss < 0.75){
  res.km<-kmeans(res.pca$ind$coord[,1:3],5)
}
#force kmeans to generate good custers
...
### Gain in inertia (in %)
After executing kmeans we get a retained inertia of the 78.88756%
```{r}
...
```
We get a p-value lower than 0.05 so we can reject the H0.
