...
```{r}
res.pca<-PCA(df[,c(vars_res,vars_con, vars_dis)], quanti.sup = 1, quali.sup = c(2,8:11), graph = FALSE)
plot(res.pca$ind$coord[,1], res.pca$ind$coord[,2], pch=19,col="darkblue", main="PCA of variables with MvOut")
points(res.pca$ind$coord[llmout,1], res.pca$ind$coord[llmout,2], col="red", pch=20)
legend("topright", legend=c("MvOut.No", "MvOut.Yes"),
       col=c("darkblue", "red"), pch=19, cex=0.7,
       title="Point types")
```
